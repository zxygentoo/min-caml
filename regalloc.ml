...
and source' t = function
  ...
  | CallCls _ | CallDir _ ->
    (match t with Type.Unit -> []| Type.Float -> [fregs.(0)] | _ -> [regs.(0)])
  | _ -> []
...
type alloc_result =
  ...
  (* spilled variable *)
  | Spill of Id.t
let alloc cont regenv x t prefer =
...
and g' dest cont regenv = function
  ...
  | StDF(x, y, z', i) ->
    (Ans(StDF(find x Type.Float regenv, find y Type.Int regenv, find' z' regenv, i)), regenv)
...
and g'_if dest cont regenv _exp constr e1 e2 =
  ...
  (List.fold_left
     ...
     (fv cont),
   regenv')
and g'_call dest cont regenv _exp constr ys zs =
  (List.fold_left
     (fun e x ->
